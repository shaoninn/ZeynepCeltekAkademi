 ...
 * Body: { orderNo, phone } — creates PayTR iframe token for an existing kayıt.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (order.paymentStatus === "PAID") {
      return NextResponse.json(
        { error: "Bu kayıt için ödeme zaten alınmış." },
        { status: 400 }
      );
    }
    if (order.status === "CANCELLED") {
      return NextResponse.json(
        { error: "İptal edilmiş kayıt için ödeme yapılamaz." },
        { status: 400 }
      );
    }
    ...
    const result = await requestPaytrIframeToken({
      ...
      userAddress: order.address || "Adana — Cemalpaşa",
